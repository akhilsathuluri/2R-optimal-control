function [cineq, ceq] = nlconst_euler(y0, h, w, N, n, m)
cineq = [];
...
ceq(5, 1) = w(m*N+5)-y0(5)-(0.5*(y0(3)^2+y0(4)^2))*h;
for i = 1:N-1
    ...
    ceq(end+1, 1) = w(m*N+i*n+3)-w(N+n*(i-1)+3)-w(m*N+4+n*(i-1))*h;
    ceq(end+1, 1) = w(m*N+i*n+4)-w(N+n*(i-1)+4)-((t5+t6*cos(w(m*N+3+(i-1)*n)))*(w(N+i)-t6*w(m*N+2+(i-1)*n)^2*sin(w(m*N+3+(i-1)*n)))-(t7+t6*cos(w(m*N+3+(i-1)*n)))*(w(i)-w(N+i)+t6*(w(m*N+2+(i-1)*n)+w(m*N+4+(i-1)*n))^2*sin(w(m*N+3+(i-1)*n))))/(t7*t5-t6^2*(cos(w(m*N+3+(i-1)*n)))^2)*h;
    ceq(end+1, 1) = w(m*N+i*n+5)-w(N+n*(i-1)+5)-(0.5*(w(m*N+3+(i-1)*n)^2+w(m*N+4+(i-1)*n)^2))*h;
end
ceq(end+1, 1) = w(end)-w(end-n)-(0.5*(w(end-2)^2+w(end-1)^2))*h;
ceq(end+1, 1) = w(end-4);
ceq(end+1, 1) = w(end-3);
ceq(end+1, 1) = w(end-2);
ceq(end+1, 1) = w(end-1);
